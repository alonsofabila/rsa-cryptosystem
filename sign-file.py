...
hashed_msg = hashlib.sha256(b'Y la de chambear no se la saben?')
print(f'\nhashed_msg: {hashed_msg}')

message = int.from_bytes(hashed_msg.digest(), byteorder='big')
signatureAlice = pow(message, dAlice, nAlice)
print(f'Firma de alice: {signatureAlice}')

received = pow(signatureAlice, e, nAlice)
print(f'Mensaje recibido de bob: {received}')

if received == message:
    print('La firma es valida')
else:
    print('ERROR: las firma no es valida')
